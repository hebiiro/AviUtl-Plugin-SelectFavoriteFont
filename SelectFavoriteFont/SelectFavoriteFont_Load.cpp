...
HRESULT loadSetting(const MSXML2::IXMLDOMElementPtr& element)
{
	...
	for (int i = 0; i < c; i++)
	{
		...
		getPrivateProfileReal(settingElement, L"containerWidthScale", g_containerWidthScale);
		getPrivateProfileReal(settingElement, L"containerHeightScale", g_containerHeightScale);
		...
		MY_TRACE_REAL(g_containerWidthScale);
		MY_TRACE_REAL(g_containerHeightScale);
